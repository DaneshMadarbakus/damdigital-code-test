...
function changeBoxColor (){
  if (this.checked) {
    $(this).parent().parent().css('background-color', '#db4646');
  } else {
    $(this).parent().parent().css('background-color', '#8ac471');
  }
}
function openTextArea (){
  $('.newObjectiveInput').show();
  $('.newObjectiveCancel').on('click', closeTextArea);
  $('.newObjectiveAdd').off('click').on('click', addNewObjective);
}
function closeTextArea (){
  $('.newObjectiveInput').hide();
}
function addNewObjective (){
  const title = $('.newObjectiveTitle').val();
  const content = $('.newObjectiveContent').val();
...
function checkDevice(){
  ...
  if (isMobile || $(window).width() <= 991){
    $('.objective').css('width', '98%');
  } else {
    $('.objective').css('width', '32.3%');
  }
}
